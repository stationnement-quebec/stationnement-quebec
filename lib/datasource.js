	...
	fs.exists(path, function (exist) {
		if (exist) {
			try {
				...
				fs.unlinkSync(path);
				console.log("Could lot load " + key);
			}
			...
			if (('panneaux' in dataCache || 'bornes' in dataCache) && 'voie_pub' in dataCache){
				var streetIdMap = streetIdMap = voie_pub.getStreetsIdMap(dataCache['voie_pub']);
